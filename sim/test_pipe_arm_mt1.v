...
module tb_pipeline_p_ext;
  ...
  always @(posedge clk) begin
      //$display("[%0t]   imem_dout=0x%016h, imem_addr_mux=0x%016h, ifid_pc=0x%016h, pc=0x%016h", 
		//$time, dut.imem_dout, dut.imem_addr_mux, dut.ifid_pc, dut.pc_dbg);
		//$display("ifid_thread_id	:%d", dut.ifid_thread_id);
		//$display("is_j	:	%b, is_beqbne  %b, wb_thread_id = %b, wb_waddr = %b, memwb_thread_id = %b, mem_thread_id = %b", 
		//dut.is_j, dut.is_beqbne, dut.wb_thread_id, dut.wb_waddr, dut.memwb_thread_id, dut.mem_thread_id);
		//$display("exmem_thread_id	:	%b, ex_thread_id : %b, idex_thread_id : %b, ifid_thread_id : %b", 
		//dut.exmem_thread_id, dut.ex_thread_id, dut.idex_thread_id, dut.ifid_thread_id);
		//$display("pc_delay	: %d, if_thread_id_delay = %d, ifid_pc = %d, ifid_thread_id = %d", dut.pc_delay, dut.if_thread_id_delay, dut.ifid_pc, dut.ifid_thread_id);
		//$display("wb_waddr	: %d, wb_thread_id = %d, wb_wdata = %d", dut.wb_waddr, dut.wb_thread_id, dut.wb_wdata);
		$display("pc_delay	:	%h, if_thread_id_delay : %h, imem_dout : %h, ifid_instr	:	%h, ifid_instr : %h, ifid_pc : %h ,  ifid_thread_id : %h", 
		dut.pc_delay, dut.if_thread_id_delay, dut.imem_dout, dut.ifid_instr, dut.ifid_instr, dut.ifid_pc, dut.ifid_thread_id);
		$display("wb_thread_id : %h, wb_waddr : %h , wb_wdata : %h", dut.wb_thread_id, dut.wb_waddr, dut.wb_wdata);
  end
  // ===========================================================================
  // check_reg
  // ===========================================================================
  ...
  // MAIN
  // ===========================================================================
  integer t, r; // Loop variables for Thread and Register
  ...
  initial begin
    ...
    check_reg(4'd4, 64'h000000000000000A, "SUB R4=R2-R1=10");
        ...
        // Inner loop iterates through addresses (0 to 15)
        for (r = 0; r < (1 << 4); r = r + 1) begin
		  //#10;
            $display("  T%0d   |  R%0d  | %h", t, r, dut.u_rf.regFile[t][r]);
        end
        $display("------------------------------------");
    end
	 $stop;
    ...
    repeat(1) begin program_imem_word(addr,NOP); addr=addr+1; end   // w9-15
    ...
    repeat(1) begin program_imem_word(addr,NOP); addr=addr+1; end   // w33-39
    ...
    // Wait for some simulation time to pass so data is written
    #100; 

    $display("\n=== REGISTER FILE CONTENT DUMP ===");
    $display("Thread | Addr | Data");
    $display("------------------------------------");

    // Outer loop iterates through threads (0 to 1)
$stop;
    // =========================================================================
    ...
    repeat(1) begin program_imem_word(addr,NOP); addr=addr+1; end   // w17-23

    program_imem_word(addr, f_slt(4'd4,4'd2,4'd1));   addr=addr+1; // w24
    repeat(1) begin program_imem_word(addr,NOP); addr=addr+1; end   // w25-31
    ...
    program_imem_word(addr, f_mov(4'd2, 8'd7));        addr=addr+1; // w2
    repeat(1) begin program_imem_word(addr,NOP); addr=addr+1; end   // w3

    // w16: BEQ R1,R2  offset=+3 ? target=9
    program_imem_word(addr, f_beq(4'd1,4'd2,16'd3));  addr=addr+1; // w4

    // w17-20: 4 mandatory NOPs after BEQ
    repeat(2) begin program_imem_word(addr,NOP); addr=addr+1; end   // w5-6

    // w21-22: poison (should be skipped by branch)
    program_imem_word(addr, f_add_i(4'd3,4'd3,8'd1)); addr=addr+1; // w7 POISON
    program_imem_word(addr, f_add_i(4'd3,4'd3,8'd1)); addr=addr+1; // w8 POISON

    // w23: landing  (16+2+5=23 ?)
    if (addr !== 9'd23) $display("WARNING: BEQ-A landing not at w23, got %0d", addr);
    program_imem_word(addr, f_mov(4'd4, 8'hAA));       addr=addr+1; // w9 LAND
   // repeat(7) begin program_imem_word(addr,NOP); addr=addr+1; end   // w24-30

    // Case B setup
    program_imem_word(addr, f_mov(4'd5, 8'd3));        addr=addr+1; // w10
    //repeat(7) begin program_imem_word(addr,NOP); addr=addr+1; end   // w32-38

    program_imem_word(addr, f_mov(4'd6, 8'd9));        addr=addr+1; // w11
    repeat(2) begin program_imem_word(addr,NOP); addr=addr+1; end   // w12-14

    // w47: BEQ R5,R6  not taken (3!=9)
    program_imem_word(addr, f_beq(4'd5,4'd6,16'd6));  addr=addr+1; // w15

    // w48-51: 4 mandatory NOPs after BEQ
    repeat(2) begin program_imem_word(addr,NOP); addr=addr+1; end   // w16-18
$stop;
    // w52: fall-through MUST execute
    program_imem_word(addr, f_add_i(4'd7,4'd0,8'h55)); addr=addr+1; // w19
    drain_and_run(40);
		//$stop;
    ...
    repeat(2) begin program_imem_word(addr,NOP); addr=addr+1; end   // w2-3

    program_imem_word(addr, f_mov(4'd2, 8'd9));        addr=addr+1; // w4
    repeat(2) begin program_imem_word(addr,NOP); addr=addr+1; end   // w5-7

    // w16: BNE R1,R2  offset=+3 ? target=23
    program_imem_word(addr, f_bne(4'd1,4'd2,16'd3));  addr=addr+1; // w8

    // w17-20: 4 mandatory NOPs after BNE
    repeat(2) begin program_imem_word(addr,NOP); addr=addr+1; end   // w9-10

    // w21-22: poison (should be skipped by branch)
    program_imem_word(addr, f_add_i(4'd8,4'd8,8'd1)); addr=addr+1; // w11 POISON
    program_imem_word(addr, f_add_i(4'd8,4'd8,8'd1)); addr=addr+1; // w12 POISON

    // w23: landing  (16+2+5=23 ?)
    if (addr !== 9'd23) $display("WARNING: BNE-A landing not at w23, got %0d", addr);
    program_imem_word(addr, f_mov(4'd3, 8'hBB));       addr=addr+1; // w13 LAND
    //repeat(7) begin program_imem_word(addr,NOP); addr=addr+1; end   // w24-30

    // Case B setup
    program_imem_word(addr, f_mov(4'd4, 8'd6));        addr=addr+1; // w14
    //repeat(7) begin program_imem_word(addr,NOP); addr=addr+1; end   // w32-38

    program_imem_word(addr, f_mov(4'd5, 8'd6));        addr=addr+1; // w15
    repeat(2) begin program_imem_word(addr,NOP); addr=addr+1; end   // w16-18

    // w47: BNE R4,R5  not taken (6==6)
    program_imem_word(addr, f_bne(4'd4,4'd5,16'd6));  addr=addr+1; // w19

    // w48-51: 4 mandatory NOPs after BNE
    repeat(2) begin program_imem_word(addr,NOP); addr=addr+1; end   // w20-21

    // w52: fall-through MUST execute
    program_imem_word(addr, f_add_i(4'd6,4'd0,8'hCC)); addr=addr+1; // w23
    drain_and_run(40);
    ...
    repeat(2) begin program_imem_word(addr,NOP); addr=addr+1; end   // w1-2

    // w8: J target=20
    program_imem_word(addr, f_j(9'd13));               addr=addr+1; // w3

    // w9-12: 4 mandatory NOPs after J
    repeat(2) begin program_imem_word(addr,NOP); addr=addr+1; end   // w4-5
    ...
    program_imem_word(addr, f_mov(4'd2, 8'hDD));       addr=addr+1; // w13 LAND
	 //$stop;
    drain_and_run(40);
