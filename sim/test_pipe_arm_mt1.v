...
module tb_pipeline_p_ext;
  ...
  always @(posedge clk) begin
      $display("[%0t]   imem_dout=0x%016h, imem_addr_mux=0x%016h, dec_branch_target= 0x%016h, ifid_pc=0x%016h, if_beq_off=0x%016h, if_off24==0x%016h,ex_branch_target=0x%016h, pc=0x%016h", 
		...
		$display("is_j	:	%b, is_beqbne  %b, wb_thread_id = %b, wb_waddr = %b, memwb_thread_id = %b, mem_thread_id = %b", dut.is_j, dut.is_beqbne, dut.wb_thread_id, dut.wb_waddr, dut.memwb_thread_id, dut.mem_thread_id);
		$display("exmem_thread_id	:	%b, ex_thread_id : %b, idex_thread_id : %b, ifid_thread_id : %b", dut.exmem_thread_id, dut.ex_thread_id, dut.idex_thread_id, dut.ifid_thread_id);
